...
import os.path
import getpass
import base.GitHub_Licenses_Database_Insert_File_201703181748.Main
...
#            db_path = os.path.join(self.db_dir_path, 'GitHub.Languages.sqlite3')
...
#                self.db_files[db]['Inserter'](db_path)
    ...
    def __CreateLicenses(self, db_path):
        subprocess.call(shlex.split("bash ./base/GitHub.Licenses.Database.Create.201703140852/Licenses/Create.sh \"{0}\"".format(db_path)))
        subprocess.call(shlex.split("bash ./base/GitHub.Licenses.Gnu.Database.Create.201703140852/Licenses/Create.sh \"{0}\"".format(db_path)))
        
    def __InsertLicenses(self):
        creator_license = self.__LicenseCreator()
#        creator_license = self.__LicenseCreator(db_dir_path)
...
#        path_db_license = '/media/{0}/{1}/{2}/GitHub.Licenses.sqlite3'.format(os_user_name, device_name, path_db_base)
        ...
        return base.GitHub_Licenses_Database_Insert_File_201703181748.Main.Main(github_user_name, path_db_account, path_db_repo, path_db_license)
